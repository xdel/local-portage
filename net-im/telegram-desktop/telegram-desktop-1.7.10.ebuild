...
if [[ -z ${PV%%*9999} ]]; then
	...
	SRC_URI="
		https://codeload.github.com/telegramdesktop/${MY_PN}/tar.gz/v${PV} -> ${P}.tar.gz
		mirror://debian/pool/main/t/${PN}/${MY_DEB}.tar.xz
		https://codeload.github.com/telegramdesktop/${MY_CRL%-*}/tar.gz/${MY_CRL##*-}
		-> ${MY_CRL}.tar.gz
		https://codeload.github.com/Microsoft/${MY_GSL%-*}/tar.gz/${MY_GSL##*-}
		-> ${MY_GSL}.tar.gz
		https://codeload.github.com/telegramdesktop/${MY_QTL%-*}/tar.gz/${MY_QTL##*-}
		-> ${MY_QTL}.tar.gz
		https://codeload.github.com/Tencent/${MY_RJS%-*}/tar.gz/${MY_RJS##*-}
		-> ${MY_RJS}.tar.gz
		https://codeload.github.com/mapbox/${MY_VAR%-*}/tar.gz/${MY_VAR##*-}
		-> ${MY_VAR}.tar.gz
		https://codeload.github.com/Cyan4973/${MY_XXH%-*}/tar.gz/${MY_XXH##*-}
		-> ${MY_XXH}.tar.gz
		test? (
			https://codeload.github.com/catchorg/${MY_CAT%-*}/tar.gz/${MY_CAT##*-}
			-> ${MY_CAT}.tar.gz
		)
	"
	RESTRICT="primaryuri"
...
src_prepare() {
	...
	local mygypargs=(
		...
		-Dapi_id=${TELEGRAM_API_ID:-50322}
		-Dapi_hash=${TELEGRAM_API_HASH:-9ff1a639196c0779c86dd661af8522ba}
