...
if [[ ${PV} == *9999* ]]; then
	...
	RELEASE_URI="http://www.splitted-desktop.com/static/libva/mplayer-vaapi/mplayer-vaapi-latest-FULL.tar.bz2"
fi
SRC_URI="${RELEASE_URI}
	!truetype? ( ${FONT_URI} )"
...
src_prepare() {
	if [[ ${PV} = *9999* ]]; then
		...
		subversion_wc_info
		sed -i -e "s/UNKNOWN/${ESVN_WC_REVISION}/" "${S}//var/tmp/portage/media-video/mplayer-vaapi-20110127/work/mplayer-vaapi-20110127/mplayer-vaapi/version.sh" || die
	else
		MPLAYER_REVISION="vaapi-20110127"
		# Set version #
		sed -i -e "s/UNKNOWN/${MPLAYER_REVISION}/" "${S}/mplayer-vaapi/version.sh" || die
	fi
	...
	epatch ../patches/mplayer-vaapi.patch
	epatch ../patches/mplayer-vaapi-gma500-workaround.patch
	epatch ../patches/mplayer-vaapi-0.29.patch
...
src_install() {
	...
	insinto /etc/mplayer
	newins "${S}/mplayer-vaapi/etc/example.conf" mplayer.conf
	cat >> "${ED}/etc/mplayer/mplayer.conf" << _EOF_
# Config options can be section specific, global
# options should go in the default section
[default]
_EOF_
	doins "${S}/mplayer-vaapi/etc/input.conf"
	if use osdmenu; then
		doins "${S}/mplayer-vaapi/etc/menu.conf"
	fi
...
_EOF_
	...
	newbin "${S}/mplayer-vaapi/TOOLS/midentify.sh" midentify
	echo "HI: ${ED}"
	cd "${ED}/usr/bin"
