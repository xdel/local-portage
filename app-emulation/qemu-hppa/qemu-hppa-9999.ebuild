...
if [[ ${PV} = *9999* ]]; then
	EGIT_REPO_URI="https://github.com/hdeller/qemu-hppa.git"
	inherit git-2
	SRC_URI=""
	EGIT_BRANCH="hppa-softmmu"
fi
...
COMMON_TARGETS="aarch64 alpha arm cris i386 hppa m68k microblaze microblazeel 
mips mips64 mips64el mipsel nios2 or1k ppc ppc64 s390x sh4 sh4eb sparc sparc64
x86_64"
...
src_install() {
	...
	if [[ -n ${softmmu_targets} ]]; then
		...
		rm -r "${ED}/usr/share/qemu/"
		rm -r "${ED}/usr/share/man/"
		rm "${ED}/usr/bin/qemu-io"
		rm "${ED}/usr/bin/ivshmem-client"
		rm "${ED}/usr/bin/ivshmem-server"
		rm "${ED}/usr/bin/qemu-img"
		rm "${ED}/usr/bin/qemu-nbd"
		rm "${ED}/usr/libexec/qemu-bridge-helper"
		if use qemu_softmmu_targets_x86_64 || use qemu_softmmu_targets_i386; then
