...
# 	Install files
	doins jetty-i2psnark.xml blocklist.txt launch-i2psnark hosts.txt || die
	doexe eepget i2prouter launch-i2psnark osid postinstall.sh runplain.sh *.config || die
	if use x86; then
...
# 	Install files to package lib
	...
	if use x86; then
		...
		lib/wrapper/linux/wrapper.jar || die
	elif use amd64; then
		doexe lib/wrapper/linux64/libwrapper.so \
		lib/wrapper/linux64/wrapper.jar || die
	fi
